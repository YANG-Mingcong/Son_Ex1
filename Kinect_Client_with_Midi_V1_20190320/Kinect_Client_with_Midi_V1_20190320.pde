...
void draw() 
{
  ...
  if (c.available() > 0) {
    ...
    stroke(0);
    line(data[0]*kReSize, data[1]*kReSize, data[2]*kReSize, data[3]*kReSize,data[4]*kReSize,data[5]*kReSize);
    ...
    MA1.changeNC(int(data[7]));
    
    
    int hSize = int(data[6]);
    int rhVol;
    if (data[8]<1){rhVol=int(data[8]*66)+30;}else{if (data[8]>=1&& data[8]<2){rhVol=int((data[8]-1)*30)+96;}else{rhVol=127;}}
